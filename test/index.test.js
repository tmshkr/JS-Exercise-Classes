...
describe("Instance of Instructor", () => {
  ...
  it("[9] can grade a student", () => {
    expect(instructor.grade({ name: "petar" }, "redux")).to.include(
      "redux",
      "petar"
    );
  });
...
describe("Instances of Student", () => {
  ...
  it("[12] has a graduate method", () => {
    expect(student.__proto__.graduate).to.be.not.undefined;
  });
  it("[13] the graduate method works correctly", () => {
    expect(
      student.__proto__.graduate.call({ grade: 70, sprintsAttempted: 33 })
    ).to.equal(true);
    expect(
      student.__proto__.graduate.call({ grade: 70, sprintsAttempted: 34 })
    ).to.equal(false);
    expect(
      student.__proto__.graduate.call({ grade: 69, sprintsAttempted: 33 })
    ).to.equal(false);
  });
});
